...
function getScheduleAvailableUsers(req, res) {
  ScheduleRepository.getSchedule().then(fullSchedule => {
      ...
      .then((users) => {
        ...
        users.forEach((user, index) => {
          ...
          fullSchedule.forEach((itemFullSchedule, indexFullSchedule) => {
            let existSchedule = existInScheduleOfUser(userSchedule, itemFullSchedule)
            if (!existSchedule)
...
function getDisponibilityByUserId(req, res) {
  ...
  UserRepository.getMeetingsByUserId(id)
    .then((data) => {
      ...
      ScheduleRepository.getSchedule().then(schedule => {
        let userAvailableSchedule = diferenceSchedule(schedule, userScheduleParsed)
...
function diferenceSchedule(schedule, userSchedule) {
  return schedule.map(itemSchedule => itemSchedule.hour).filter(item => {
...
export default { listUsers, findById, getMeetingsByUserId, getDisponibilityByUserId, getScheduleAvailableUsers };